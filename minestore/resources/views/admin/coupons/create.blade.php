...
<h4 class="fw-bold py-3 mb-1">
  <span class="text-body fw-light">{{ __('New Coupon') }}
</span></h4>
...
<div class="col-12 mb-4">
			...
			<div class="row">
					...
					<div class="col-md-4 mb-3">
						...
						<select class="selectpicker w-100 show-tick" id="type" name="type" data-icon-base="bx" data-tick-icon="bx-check" data-style="btn-default">
						  <option data-icon="bxs-offer" value="0" selected>{{ __('Percentage') }}</option>
						  <option data-icon="bx-money" value="1">{{ __('Amount') }}</option>
						</select>
					</div>
					<div class="col-sm-4 mb-3" id="discount_percent_block">
						...
						<div class="input-group">
						  <input type="number" class="form-control" id="discount_percent" name="discount_percent" value="10" aria-label="Amount to discount original price') }}">
					...
					<div class="col-sm-4 mb-3" id="discount_money_block" style="display:none">
						...
						<div class="input-group">
						  <input type="text" inputmode="numeric" pattern="^\d*([,.]\d{1,2})?$" class="form-control" id="discount_money" name="discount_money" value="10" aria-label="Amount to discount original price">
					...
					<div class="col-sm-4 mb-3">
						...
						<div class="input-group">
						  <input type="number" id="available" name="available" class="form-control" value="10" aria-label="Max uses amount.">
						</div>
					</div>
					<div class="col-sm-4 mb-3">
						...
						<div class="input-group">
						  <input type="number" class="form-control" id="limit_per_user" name="limit_per_user" value="0" aria-label="Redeem Limit Per Customer/IP.">
						</div>
					</div>
					<div class="col-sm-4 mb-3">
						...
						<div class="input-group">
						  <input type="number" class="form-control" id="min_basket" name="min_basket" value="0" aria-label="Minimum Basket Value.">
						  <span class="input-group-text">{{ $settings->currency }}</span>
						</div>
					</div>
					<div class="col-sm-4 mb-3">
						...
						<select id="apply_type" name="apply_type" class="selectpicker w-100" data-style="btn-default">
							<option value="0" selected>{{ __('Whole Webstore') }}</option>
							<option value="1">{{ __('Categories') }}</option>
							<option value="2">{{ __('Packages') }}</option>
						</select>
					</div>
					...
					<div class="col-sm-6 mb-3">
						...
						<input type="text" class="form-control" id="start_at" name="start_at" placeholder="YYYY-MM-DD HH:MM" />
                        @error('start_at')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
	...
	<div class="row">
		<div class="d-grid gap-2 col-lg-12 mx-auto">
			<button class="btn btn-primary btn-lg" type="submit"><span class="tf-icon bx bx-plus-circle bx-xs"></span> {{ __('Create a Coupon') }}</button>
		</div>
	</div>
